...
horizon.forms = {
  ...
  datepicker: function() {
    var startDatepicker;
    if($('input#id_start_date').length > 0){
    	startDatepicker = $('input#id_start_date').datepicker({todayBtn:1,autoclose:1,startView:2});
    }else{
    	startDatepicker = $('input#id_start').datepicker({todayBtn:1,autoclose:1,minView:2});
    }
    var startDate = startDatepicker.on('changeDate', function(ev) {
        if (ev.date.valueOf() > endDate.date.valueOf()) {
          var newDate = new Date(ev.date);
          newDate.setDate(newDate.getDate() + 1);
          endDate.setValue(newDate);
          if($('input#id_end').length > 0){
		    $('input#id_end')[0].focus();
		  }else{
		    $('input#id_end_date')[0].focus();
		  }
        }
      }).data('datepicker');
	
	var endDatepicker;
	if($('input#id_end_date').length > 0){
    	endDatepicker = $('input#id_end_date').datepicker({todayBtn:1,autoclose:1,startView:2});
    }else{
    	endDatepicker = $('input#id_end').datepicker({
	      onRender: function(date) {
	        return date.valueOf() < startDate.date.valueOf() ? 'disabled' : '';
	      },todayBtn:1,autoclose:1,minView:2
	    })
    }
    var endDate = endDatepicker.on('changeDate', function(ev) {
      }).data('datepicker');
  }
};
