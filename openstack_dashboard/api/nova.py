...
def server_update(request, instance_id, name, metadata=None):
    if metadata:
        try:
            server_set_meta(request, instance_id, metadata)
        except Exception:
            return False
    return novaclient(request).servers.update(instance_id, name=name)


def server_set_meta(request, instance_id, metadata=None):
