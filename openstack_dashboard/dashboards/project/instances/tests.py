...
class InstanceTests(test.TestCase):
    ...
    def test_instance_details_metadata(self):
        ...
        self.assertContains(res,
                            "<dt title=\"someMetaLabel\">someMetaLabel</dt>",
                            1)
        self.assertContains(res, "<dd>someMetaData</dd>", 1)
        self.assertContains(res,
                            "<dt title=\"some&lt;b&gt;html&lt;/b&gt;label\">"
                            "some&lt;b&gt;html&lt;/b&gt;label</dt>",
                            1)
        self.assertContains(res, "<dd>&lt;!--</dd>", 1)
        self.assertContains(res,
                            "<dt title=\"empty\">empty</dt>", 1)
    ...
    @test.create_stubs({api.glance: ('image_list_detailed',),
                        ...
                        cinder: ('volume_list',
                                 'volume_snapshot_list',),
                        quotas: ('tenant_quota_usages',)})
    def test_launch_instance_post_reboot(self):
        flavor = self.flavors.first()
        image = self.images.first()
        keypair = self.keypairs.first()
        server = self.servers.first()
    ...
    @test.create_stubs({api.glance: ('image_list_detailed',),
                        ...
                        cinder: ('volume_list',
                                 'volume_snapshot_list',),
                        quotas: ('tenant_quota_usages',)})
    def test_launch_instance_post_boot_from_volume_reboot(self):
        flavor = self.flavors.first()
        keypair = self.keypairs.first()
        server = self.servers.first()
        volume = self.volumes.first()
    ...
    @test.create_stubs({api.glance: ('image_list_detailed',),
                        ...
                        cinder: ('volume_list',
                                 'volume_snapshot_list',),
                        quotas: ('tenant_quota_usages',)})
    def test_launch_instance_post_no_images_available_boot_from_reboot(self):
        flavor = self.flavors.first()
        keypair = self.keypairs.first()
        server = self.servers.first()
        volume = self.volumes.first()
    ...
    @test.create_stubs({api.glance: ('image_list_detailed',),
                        ...
                        cinder: ('volume_list',
                                 'volume_snapshot_list',),
                        quotas: ('tenant_quota_usages',)})
    def test_launch_form_keystone_reboot(self):
        flavor = self.flavors.first()
        image = self.images.first()
        keypair = self.keypairs.first()
        server = self.servers.first()
