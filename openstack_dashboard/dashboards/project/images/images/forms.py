...
class CreateImageForm(forms.SelfHandlingForm):
    ...
    template_name = forms.CharField(max_length="255",
                                    ...
                                    required=False)
    size = forms.CharField(label=_("Image size"),
                                 help_text=_("Size(KB, MB, GB) of this image "
                                             "in vmware, such as 25MB, 1GB."),
                                 widget=forms.TextInput(attrs={
                                     'class': 'switched',
                                     'data-switch-on': 'source',
                                     'data-source-vmware': _('Image size')}),
                                 required=False)
    ...
    hyper_type = forms.ChoiceField(
        ...
        choices=[('', ''),
                 ...
                 ('powervm', _('powervm'))],
        widget=forms.Select(attrs={'class': 'switchable'}),
        help_text=_('powervm must be used Raw format'),
        required=False
    )
    ...
    def handle(self, request, data):
        ...
        have_size = data.get('size', None)
        if have_size:
            size = have_size.upper()
            if size.find('K') > 0:
                size = int(size.split('K')[0]) * 1024
            elif size.find('M') > 0:
                size = int(size.split('M')[0]) * 1024 * 1024
            elif size.find('G') > 0:
                size = int(size.split('G')[0]) * 1024 * 1024 * 1024
            else:
                size = have_size
        ...
        if data['hyper_type']:
            meta['properties']['hypervisor_type'] = data['hyper_type']
